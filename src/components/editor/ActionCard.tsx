...
export function ActionCard({ action, onToggle, onPreview }: ActionCardProps) {
  ...
  const reasonLabel = action.reason
    .replace("false_start", "False Start")
    .replace("repeated_take", "Repeated Take")
    .replace("redundant_sentence", "Redundant")
    .replace("important_statement", "Highlight")
    .replace("noise_only", "Background Noise");
      ...
      <CardContent className="flex items-center justify-between p-4">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <span className="font-semibold">{durationSec}s {isCut ? "Cut" : isHighlight ? "Highlight" : "Action"}</span>
            <Badge variant={getBadgeVariant()}>{reasonLabel}</Badge>
            <Badge variant="outline" className="text-xs uppercase">{sourceLabel}{action.isManualModified ? " (Mod)" : ""}</Badge>
            {action.confidence !== null && action.source === "ai" && (
              <span className="ml-1 text-xs text-muted-foreground">{(action.confidence * 100).toFixed(0)}% confident</span>
            )}
          </div>
          <span className="text-sm text-muted-foreground">{formatTime(action.startMs)} - {formatTime(action.endMs)}</span>
        </div>
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" onClick={() => onPreview(action.startMs, action.endMs)}>
